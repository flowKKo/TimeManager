            ...
            case R.id.button_save:
                if(((EditText)getView().findViewById(R.id.editText_editTodo)).getText().length()==0){
                    ...
                    if(todo.getIs_clock()){//更新系统日历event
                        ...
                        SysCalendar.updateCalendarEvent(getContext(),"Todo"+Integer.toString(todo.getId()),
                                todo.getTodo(),date,time);//更新系统日历的event
                    }
                    else{//删除系统日历event
                        SysCalendar.deleteCalendarEvent(getContext(),"Todo"+Integer.toString(todo.getId()));
                    }
